const validateTrainSearch = (query) => {
  const { source, destination, date, passengers, minPrice, maxPrice, sortBy, page, limit } = query;

  if (!source || typeof source !== "string" || !source.trim()) {
    return { valid: false, message: "Source station (ID, code, or name) is required" };
  }

  if (!destination || typeof destination !== "string" || !destination.trim()) {
    return { valid: false, message: "Destination station (ID, code, or name) is required" };
  }
  ...
  const allowedSort = ["price_low", "price_high", "departure_early", "departure_late"];
